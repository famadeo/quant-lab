...
- **Pre-declared variant set (no others):** best `target3m_minv10`, liquid `target1m_minv500`, liquid `target1m_minv1000`. Each short name resolves to the **single best FULL variant** = the `strategy_metrics.csv` row with maximum `net_return` among rows where `variant.startswith(short + "_")` and `cost_multiplier == 1.0` (this is the lab's own "best variant" rule). Events must be filtered to that **exact** full-variant string, never a prefix match — a prefix sweeps in entry2.0 and side-only variants and double-counts. Read `lookback`/`entry_z`/`exit_z` from that row (do not hard-code; the best lookback differs between 0030 lb252 and 0031 lb126).
...
- Produces:
  ...
  - `load_variant(source_dir, variant_name) -> tuple[pd.DataFrame, pd.DataFrame, dict]` — resolves `variant_name` to the single best full variant (max `net_return`, cost 1.0), returns `(z_panel, events, params)` where `events` is the `event_log` rows whose `variant` **exactly equals** that full-variant string, and `params` holds `{variant,target_months,min_volume,lookback,entry_z,exit_z}` (the full-variant string under key `variant`).
