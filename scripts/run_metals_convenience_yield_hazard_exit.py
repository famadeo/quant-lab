...
OUTPUT_DIR = REPO_ROOT / "experiments" / "HYP-0033-metals-convenience-yield-hazard-exit"
BASELINE_METRICS = (
    REPO_ROOT
    / "experiments"
...
HAZARD_CANDIDATE_BARS = [5, 8, 13, 21, 34]
HAZARD_MIN_HISTORY_EVENTS = 8
HAZARD_MIN_TAIL_EVENTS = 4
HAZARD_MAX_TAIL_WIN_RATE = 0.50
...
class RobustVariant:
    ...
    hazard_exit: bool = True
    hazard_min_history_events: int = HAZARD_MIN_HISTORY_EVENTS
    hazard_min_tail_events: int = HAZARD_MIN_TAIL_EVENTS
    ...
    @property
    def name(self) -> str:
        if not self.hazard_exit:
            return self.base_name
        return (
            f"{self.base_name}_hazardexp_minh{self.hazard_min_history_events}"
            f"_tail{self.hazard_min_tail_events}"
        )
...
def event_side(position: int) -> str:
    return "fade_backwardation" if position > 0 else "fade_contango"


def stale_exit_profile(
    completed_events: list[dict[str, object]], side: str, variant: RobustVariant
) -> dict[str, object]:
    profile: dict[str, object] = {
        "stale_exit_bar": np.nan,
        "hazard_history_events": 0,
        "hazard_tail_events": 0,
        "hazard_tail_mean_net_return": np.nan,
        "hazard_tail_win_rate": np.nan,
    }
    if not variant.hazard_exit:
        return profile

    side_events = [
        event
        for event in completed_events
        if event.get("side") == side and not bool(event.get("forced_exit", False))
    ]
    profile["hazard_history_events"] = len(side_events)
    if len(side_events) < variant.hazard_min_history_events:
        return profile

    history = pd.DataFrame(side_events)
    for candidate in HAZARD_CANDIDATE_BARS:
        tail = history[history["holding_days"] >= candidate]
        if len(tail) < variant.hazard_min_tail_events:
            continue
        tail_net = tail["weighted_net_return"]
        tail_mean = float(tail_net.mean())
        tail_win_rate = float((tail_net > 0).mean())
        if tail_mean <= 0.0 and tail_win_rate <= HAZARD_MAX_TAIL_WIN_RATE:
            profile.update(
                {
                    "stale_exit_bar": candidate,
                    "hazard_tail_events": len(tail),
                    "hazard_tail_mean_net_return": tail_mean,
                    "hazard_tail_win_rate": tail_win_rate,
                }
            )
            break
    return profile
...
def open_event(
    *,
    ...
    side = event_side(position)
    return {
        "variant": variant.name,
        "base_variant": variant.base_name,
        ...
        "holding_days": 0,
        "rolls": 0,
        **stale_exit_profile(completed_events, side, variant),
    }


def maybe_apply_hazard_exit(
    current_event: dict[str, object] | None,
    previous_position: int,
    new_position: int,
    exit_reason: str,
) -> tuple[int, str]:
    if not current_event or previous_position == 0 or new_position != previous_position:
        return new_position, exit_reason
    stale_exit_bar = current_event.get("stale_exit_bar", np.nan)
    if np.isfinite(stale_exit_bar) and int(current_event["holding_days"]) >= int(
        stale_exit_bar
    ):
        return 0, "hazard_stale"
    return new_position, exit_reason


def close_event(
    event: dict[str, object], row: pd.Series, variant: RobustVariant, exit_reason: str
) -> dict[str, object]:
    event["exit_date"] = row["date"]
    event["exit_signal"] = signal_value(row, variant)
    event["exit_reason"] = exit_reason
    event["hazard_exit"] = exit_reason == "hazard_stale"
    return event
...
def accrue_event(
    *,
    ...
    if not event or new_position == 0:
        return
    event["gross_spread_return"] = float(event["gross_spread_return"]) + new_position * float(
        row["spread_return"]
    )
    event["weighted_gross_return"] = float(event["weighted_gross_return"]) + gross
    event["weighted_cost_return"] = float(event["weighted_cost_return"]) + leg_cost
    event["weighted_net_return"] = float(event["weighted_net_return"]) + net
    event["holding_days"] = int(event["holding_days"]) + 1
    event["rolls"] = int(event["rolls"]) + int(pair_changed)
...
def return_row(
    *,
    ...
    return {
        ...
        "turnover": abs(new_position - previous_position) * base.ROOT_WEIGHT,
    }
...
def simulate_root(
    ...
    for record in data.to_dict("records"):
        ...
        if not np.isfinite(row["spread_return"]):
            new_position = 0
            exit_reason = "missing_return"
        new_position, exit_reason = maybe_apply_hazard_exit(
            current_event, previous_position, new_position, exit_reason
        )

        position_changed = previous_position != new_position
        if position_changed and previous_position != 0 and current_event:
            current_event = close_event(current_event, row, variant, exit_reason)
            events.append(current_event)
            completed_events.append(current_event)
            current_event = None

        if position_changed and new_position != 0:
        ...
        leg_cost = spread_leg_cost(
            previous_position=previous_position,
            new_position=new_position,
            pair_changed=pair_changed,
            leg_cost_bps=leg_cost_bps,
            variant=variant,
        )
    ...
    if current_event:
        ...
        current_event["exit_reason"] = "forced"
        current_event["hazard_exit"] = False
        current_event["forced_exit"] = True
        events.append(current_event)
    for event in events:
        event.setdefault("forced_exit", False)
        event.setdefault("hazard_exit", False)
        event.setdefault("exit_reason", "unknown")
        event["duration_days"] = (
...
def summarize_strategy(
    ...
    hazard_exit_count = int(events["hazard_exit"].sum()) if "hazard_exit" in events else 0
    return {
        "variant": variant.name,
        "base_variant": variant.base_name,
        "detector": variant.detector,
        ...
        "hazard_exit": variant.hazard_exit,
        "hazard_min_history_events": variant.hazard_min_history_events,
        "hazard_min_tail_events": variant.hazard_min_tail_events,
        ...
        "hazard_exit_count": hazard_exit_count,
        "hazard_exit_fraction": hazard_exit_count / len(events) if len(events) else np.nan,
...
def baseline_comparison(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    comparison = metrics.merge(
        ...
        left_on="base_variant",
        right_on="variant",
        how="left",
        suffixes=("", "_baseline"),
    ).drop(columns=["variant_baseline"])
    comparison["net_delta"] = comparison["net_return"] - comparison["net_return_baseline"]
    comparison["sharpe_delta"] = comparison["sharpe"] - comparison["sharpe_baseline"]
    comparison["tstat_delta"] = comparison["tstat"] - comparison["tstat_baseline"]
    comparison["cost_delta"] = comparison["cost_return"] - comparison["cost_return_baseline"]
    columns = [
        "variant",
        "base_variant",
        "detector",
        ...
        "hazard_exit_fraction",
        "event_tstat",
        "event_tstat_baseline",
        "active_fraction",
        "active_fraction_baseline",
    ]
    return comparison[columns].sort_values("net_delta", ascending=False)


def bucket_comparison(comparison: pd.DataFrame) -> pd.DataFrame:
    if comparison.empty:
        return pd.DataFrame()
    idx = comparison.groupby(["min_volume", "cost_multiplier"])["net_return"].idxmax()
    return comparison.loc[idx].sort_values(["min_volume", "cost_multiplier"])
...
def write_report(
    *,
    ...
    lines = [
        ...
        "- Adds a lagged stale-event exit: each new event looks only at previously completed "
        "same-root, same-side events for that exact variant.",
        "- If prior events that remained open beyond a candidate age had non-positive mean net "
        "return and win rate no better than 50%, the new event exits at that adaptive age if "
        "normalization has not already occurred.",
        "",
        "## Best Variant",
        "",
        ...
        "",
        comparison.head(15)[compare_cols].to_markdown(index=False, floatfmt=".4f")
        if not comparison.empty
        else "Baseline metrics were not available.",
        "",
        "## Best Hazard Variant By Volume And Cost Bucket",
        "",
        bucket_compare[compare_cols].to_markdown(index=False, floatfmt=".4f")
        ...
        "",
        (
            f"The best hazard-exit variant is `{best_variant}` with net cumulative log "
            f"return `{metrics.iloc[0]['net_return']:.4f}`, t-stat "
            f"`{metrics.iloc[0]['tstat']:.2f}`, and event t-stat "
            f"`{metrics.iloc[0]['event_tstat']:.2f}`."
        ),
        "",
        "This experiment asks whether adaptive stale exits improve net return without "
        "damaging Sharpe. The relevant decision is the paired HYP-0032 comparison, not only "
        "the best standalone HYP-0033 variant.",
        "",
        "## Files",
        "",
        "- `strategy_metrics.csv`",
...
def main() -> None:
    ...
    comparison = baseline_comparison(metrics)
    bucket_compare = bucket_comparison(comparison)
    ...
    bucket_compare.to_csv(OUTPUT_DIR / "bucket_baseline_comparison.csv", index=False)
